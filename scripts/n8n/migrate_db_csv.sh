...
for TABLE_NAME in "${TABLES[@]}"; do
    echo "Processing retention policy for table: $TABLE_NAME on source"
    
    # Identify existing retention policies for the table
    echo "Identifying existing retention policies for $TABLE_NAME on source"
    ...
    echo "Removing retention policy for $TABLE_NAME if exists on source"
...
echo "Migrating schema pre-data on source"
...
echo "Restoring the dump pre-data on target"
...
for TABLE_NAME in "${TABLES[@]}"; do
    ...
    echo "Checking if $TABLE_NAME exists on target"
    retry_command "psql -p $PGPORT_DEST -d $DB_NAME -c \"SELECT to_regclass('public.$TABLE_NAME');\""

    # Convert table to hypertable
    echo "Converting $TABLE_NAME to hypertable on target"
...
for TABLE_NAME in "${TABLES[@]}"; do
    ...
    echo "Copy data for $TABLE_NAME from source"
    ...
    echo "Checking and removing retention policy for $TABLE_NAME if exists on target"
    ...
    echo "Importing data into $TABLE_NAME on target"
